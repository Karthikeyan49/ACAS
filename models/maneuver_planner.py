# ─────────────────────────────────────────────────────────────────────────────
# models/maneuver_planner.py
#
# PURPOSE:
#   Reinforcement Learning agent trained to find the optimal thruster burn
#   (minimum fuel, maximum miss distance) for any conjunction scenario.
#
#   ManeuverEnv  — Gymnasium environment: the RL training sandbox
#   train_rl_agent() — trains PPO agent for 400,000 steps (~25 min)
#
# HOW RL WORKS HERE:
#   The agent tries a burn, sees the resulting miss distance and fuel cost,
#   gets a reward, and adjusts its policy to earn more reward next time.
#   After 400,000 episodes it has learned: "burn perpendicular to velocity,
#   just enough to get 5km clearance, no more."
#
# CALLED BY:
#   onboard/acas_controller.py — loads saved policy for inference
#
# TO TRAIN:
#   python models/maneuver_planner.py
#   Saves: trained_models/maneuver_policy.zip
# ─────────────────────────────────────────────────────────────────────────────
