...
#
# PURPOSE:
#   Takes the raw Pc from ConjunctionNet and adjusts it based on the
#   satellite's real operational state. This is where all limitations
#   are handled:
#     1. TLE staleness         → inflates Pc
#     2. Fuel level            → raises action thresholds
#     3. Battery level         → boosts sensitivity, reduces thrust
#     4. Altitude floor        → restricts burn direction
#     5. Post-maneuver path    → doubles Pc if new conjunction created
#     6. Mission phase         → adjusts sensitivity
#
#   Then classifies the adjusted Pc as GREEN/YELLOW/ORANGE/RED
#   and computes the minimum-fuel maneuver to resolve the threat.
#
# CALLED BY:
#   onboard/acas_controller.py — for every conjunction event each cycle
# ─────────────────────────────────────────────────────────────────────────────
