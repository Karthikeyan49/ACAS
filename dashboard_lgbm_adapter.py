"""
dashboard_lgbm_adapter.py
─────────────────────────
Adapter for dashboard/app.py — connects the manual threat injection
form to the LightGBM model.

Your dashboard already has a custom threat form with sliders:
    miss_km, tca_h, rel_pos[3], rel_vel[3], tle_age

This adapter wraps that into the LightGBM prediction pipeline and returns
the full result dict that your dashboard renders.

HOW TO USE IN dashboard/app.py:
    Replace the block where you call:
        raw_pc = engine.predict_pc(extract_features(conj))
        assessment = scorer.assess(conj, raw_pc, sat_state)

    With:
        from dashboard_lgbm_adapter import assess_threat_from_dashboard
        result = assess_threat_from_dashboard(threat_dict, sat_state)

    Then use result["assessment"] as before, plus result["lgbm"] for
    the richer LightGBM-specific fields.
"""
