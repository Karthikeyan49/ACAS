"""
model/lgbm_engine.py
══════════════════════════════════════════════════════════════════════════════
WHAT THIS FILE IS
    The inference bridge between the ACAS runtime and the trained LightGBM
    model. Accepts the 10-field conjunction dict that ConjunctionFinder
    produces, converts it to 103 CDM features, runs both LightGBM models,
    and returns raw_pc as a float in [0.0, 1.0].

    Drop-in replacement for the broken ONNX ConjunctionNet.
    RiskScorer, controller, and dashboard receive the same raw_pc float
    they always expected — no other code changes needed.

CALLED FROM
    core/controller.py      engine.predict_pc_from_conjunction(conj)
    dashboard/app.py        same (loaded once via @st.cache_resource)
    dashboard/adapter.py    same

CALLS INTO
    model/config.py          MODEL_DIR, CATEGORICAL_COLS
...
WHAT IT PROVIDES
    conjunction_dict_to_cdm(conj) → dict (103 columns)
        Derives RIC-frame pos/vel, sigma estimates from object type and TLE
        age, Mahalanobis distance, physical parameters, space weather defaults.

    LGBMInferenceEngine(model_dir=None)
        predict_pc_from_conjunction(conj) → float   PREFERRED
            Full 103-feature pipeline. Returns raw_pc in [0.0, 1.0].
        predict_pc(features) → float
            Backward compat. Accepts dict or 12-element np.ndarray.
        is_loaded → bool
...
MIGRATED FROM  satellite-acas/lgbm_inference_engine.py
IMPORT CHANGES
